...
class JurnalAkuntansi extends Model
{
    ...
    //jenis saldo manggil di bukbes
    public static function jenis_saldo($jenis_akun, $no_akun)
    {
        $jenis_saldo = "";
        if ($jenis_akun == "aset") { //jika aset maka debet, kecuali 111
            if ($no_akun == 111) {
                $jenis_saldo = "kredit";
            } else {
                $jenis_saldo = "debet";
    ...
    { //hitung bukbes, parameter itu periode aktif
        $buku_besar2 = []; //array simpan all bukbes buat direturn
        $buku_besar = new JurnalAkuntansi(); //declare
        $akuns = AkunAkuntansi::where('periode_id', $id_periode)->get(); //mengambil akun akuntansi sesuai periode

        $jurnals = JurnalAkuntansi::where('periode_id', $id_periode)->get(); //mengambil jurnal akuntansi sesuai periode
        // $jurnals_detail =  DB::select(DB::raw("SELECT * FROM `jurnal_has_akun`"));
        foreach ($akuns as $akun) { //foreach akun
            $tumpung['saldo_awal_kredit'] = 0;  //tampung saldo awal kredit biar value nya 0
            $tumpung['saldo_awal_debet'] = 0;
            $jenis_saldo = $buku_besar->jenis_saldo($akun->jenis_akun, $akun->no_akun); //ngecek jenis saldonya debet apa kred, berdasarkan
            //jenis akun dan no akun

            $tumpung['no_akun'] = $akun->no_akun; //simpan no akun
            $tumpung['nama_akun'] = $akun->nama;
            $tumpung['jenis_akun'] = $akun->jenis_akun;
            $tumpung['saldo_awal_' . $jenis_saldo] = $akun->saldo_awal; //ngeset saldo awal
            $tumpung['list'] = [];
            // $jumlah_sebelum_closing = $akun->saldo_awal;
            // $jumlah_setelah_closing = $akun->saldo_awal;

            $saldo_jurnal_ekses = $akun->saldo_awal; //menghitung semua jurnal umum utk buku besar
            $total_sel = 0; //simapn total sebelum closing
            $total_sesudah = 0;

            $jurnal_penutup = $akun->saldo_awal; //mengitung before and after closing
            ...
            foreach ($jurnals as $jurnal) {
                ...
                foreach ($jurnal->akun as $key =>  $jurnalakun) {
                    if ($akun->no_akun == $jurnalakun->no_akun) { //menghitung yang bukbes untuk menjumlahkan kredit debet
                        if ($no_reff == 3) { //ini yg biru highlight
                            if ($jenis_saldo == "kredit") { //saldo kredit
                                if ($jurnalakun->pivot->nominal_kredit != 0) { //jika dia tidak 0 maka dihitung untuk closingan
                                    $jurnal_penutup += $jurnalakun->pivot->nominal_kredit;
                                } else {
                                    $jurnal_penutup  -= $jurnalakun->pivot->nominal_debit; //jika 0 maka masukin debitnya karena posisi ada di debet
                                }
                            } else {
                            ...
                            $selisih = abs($saldo_jurnal_ekses) - abs($jurnal_penutup); //abs menjadikan angka positif
                            $total_sesudah = $saldo_jurnal_ekses;
                            //mencari selisih saldo sebelum dan sesudah closing
                            if ($selisih == 0) { //jika tidak ada selisih di saldo before after maka saldo sebelum closing sama dengan after
                                $total_sel = abs($jurnal_penutup);
                                $total_sesudah = abs($saldo_jurnal_ekses) - abs($jurnal_penutup);
                            } else { //nemu selisih
                                $total_sel = abs($jurnal_penutup) - abs($selisih); //maka jurnal penutup dikurangi selisih
                                $total_sesudah = $jurnal_penutup; //saldo sesudah closing dikurangi saldo selisih, jika 4m saldo awal trs ada selisih 16jt. maka saldo
                                //closing jadi 4m 16 jt
                            }
                            ...
                            if ($jenis_saldo == "kredit") { //penjurnalan sama dengan atas tapi ini jurnal umum ini
                                if ($jurnalakun->pivot->nominal_kredit != 0) {
                        ...
                        //list itu nampung transaksi untuk jurnal has akun
                    }
                }
            }
            // jika tidak ada transaksi
            if (count($tumpung['list']) == 0) { //ini yg menampilkan saldo awal tok
                $tumpung['saldo_sebelum_closing'] = abs($akun->saldo_awal); //jika sebelum closing saldonya sama dgn awal

            } else {

                $tumpung['saldo_sebelum_closing'] = abs($total_sel); //ini menampilkan jika ada transaksi di dalem
            }
            $tumpung['saldo_setelah_closing'] = abs($total_sesudah); //saldo setelah closing yg final

            array_push($buku_besar2, $tumpung);
            unset($tumpung); //unset biar ga dobel
        }
        return $buku_besar2;
    }
    ...
    { //laba rugi
        ...
        foreach ($buku_besar as $value) {
            if ($value['jenis_akun'] == "pendapatan") {
                ...
                $nama_akun = $akun->nama;
                $saldo = $value['saldo_sebelum_closing']; //saldo sebelum closing, saldo ini nampung saldo pendapatan2 perakun
                $total_pendapatan += $saldo; //total semua akun
                $pendapatan = [
                    'no_akun' => $no_akun,
                    'nama_akun' => $nama_akun,
                    'saldo' => $saldo,
        ...
        foreach ($buku_besar as $bukubesar) {
            if ($bukubesar['no_akun'] == 101) {
                ...
                foreach ($bukubesar['list'] as $list) {
                    ...
                    if (count($jurnalData->akun) > 2) { //masuk ke koreksi jika data lebih dari 2
    ...
    { //penutupan
        ...
        $buku_besar = $newjurnal->bukubesar($id_periode);
        $akuns = AkunAkuntansi::where('periode_id', $id_periode)->get();
        $iktiar = AkunAkuntansi::where('periode_id', $id_periode)->where('no_akun', 000)->first(); //buat ambil ikhtiar id karena setelah di closing jadi harus tau periode
        $iktiar_id = $iktiar->id;
        ...
        $step_1->save();
        $step_1_iktiar = 0; //nge SUM ikhtiar buat nambah di line 547
        $step_1_urut = 1; //no urut transaksi
            ...
            foreach ($buku_besar as $bukubesar) {
                if ($akun->no_akun == $bukubesar['no_akun']) {
                    ...
                    if ($akun->jenis_akun == "pendapatan") {
                        ...
                        $step_1_iktiar += $saldo_sebelum; //ditambahno semuae
                        $step_1_urut += 1; //hasil tambah 530
                    }
                }
            }
        }
    ...
    { //daftar akun baru
        $newjurnal = new JurnalAkuntansi();
        $buku_besar = $newjurnal->bukubesar($id_periode_lama); //ngeget bukbes, hitung yang periode lama
        $akuns = AkunAkuntansi::where('periode_id', $id_periode_lama)->get(); //diget dulu periode lama

        foreach ($akuns as $akun) {
            foreach ($buku_besar as $bukubesar) {
                if ($akun->no_akun == $bukubesar['no_akun']) { //jika akun dan bukbes sama maka
                    // New Akun
                    $new_akun = new AkunAkuntansi();
                    $new_akun->no_akun = $akun->no_akun;
                    $new_akun->nama = $akun->nama;
                    $new_akun->jenis_akun = $akun->jenis_akun;
                    $new_akun->saldo_awal = $bukubesar['saldo_setelah_closing']; //ngeset saldo closing untuk daftar akun periode baru
