<div class="modal-header">
    ...
    <h4 class="modal-title">Edit Nota Penjualan</h4>
</div>
    ...
    <form action="{{ route('notapembelian.update',$data->id) }}" method="post" enctype="multipart/form-data" class="form-horizontal">
        ...
        <div class="form-body">
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="customer" id="eCustomer">
                    @foreach ($customer as $item)
                    <option value="{{ $item->id }}" {{ $item->id == $data->customer_id ? 'selected' : '' }}>{{ $item->nama }}</option>
                    @endforeach
                </select>
            </div>
